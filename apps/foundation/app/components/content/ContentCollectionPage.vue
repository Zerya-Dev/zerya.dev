...
const props = defineProps<{ route: ContentRoute }>()
const page = CONTENT_ROUTES[props.route]
const { data: items } = await useCollection(page.collection)

const featuredItem = items.value?.find(item => item.featured) ?? items.value?.[0]
...
<template>
  <main id="main-content">
    <PageHero :label="page.hero.label" :title="page.hero.title" :description="page.hero.description" />
    ...
    <PageSection color="bg-canvas" label="content-archive-title">
      <SectionHeader :label="page.archive.label" :title="page.archive.title" :description="page.archive.description" title-id="content-archive-title" />
    ...
    <CtaSection :title="page.cta.title" :description="page.cta.description" :button-label="page.cta.button.label" :to="page.cta.button.to" />
  </main>
</template>
