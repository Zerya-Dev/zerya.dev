...
import { FEATURED_LINK } from '~~/content/settings'

const navigationItems = NAVIGATION_ITEMS.map(item => ({ ...item, exact: item.to === '/' }))
const navigationUi = {
  list: 'gap-4 lg:gap-6',
...
<template>
  ...
  >
    ...
    <template #right>
      <UButton
        :to="FEATURED_LINK.external ? undefined : FEATURED_LINK.to"
        :href="FEATURED_LINK.external ? FEATURED_LINK.to : undefined"
        target="_blank"
    ...
    <template #body>
      ...
      />
      <UButton :to="FEATURED_LINK.external ? undefined : FEATURED_LINK.to" :href="FEATURED_LINK.external ? FEATURED_LINK.to : undefined" target="_blank" rel="noopener noreferrer" variant="link" trailing-icon="i-lucide-arrow-up-right" class="mt-4 px-0 py-4 font-bold">
        {{ FEATURED_LINK.label }}
        <span class="sr-only">(otwiera się w nowej karcie)</span>
      </UButton>
    </template>
